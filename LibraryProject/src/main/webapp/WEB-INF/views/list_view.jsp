...
<div class="row mt-5">
	...
	<div class="col-10">
		<!-- 신간도서게시판 -->
		<div id = "maindiv">
			<h3 id = "newbookh3" class="mb-2" style="color:#527062;"><i class="fas fa-book"></i>&nbsp;<b>신간도서</b></h3>
			<hr/>
			<p style="font-size:18px; background:#ececf0;border-radius:20px 40px;" class="p-5">
				<span style="color:#527062;"><b>신간도서 소개</b></span><br/>
				2022년 신간 도서를 소개합니다.
			</p>
			<div align="right" class="mb-1 mt-5">
				<form action="newListSearch" method="post" id="newListSearch">
					<select name="type">
						<option value="title">제목</option>
						<option value="writer">저자</option>
					</select>
					<input type="text" name="keyword" value="" placeholder="키워드 검색"/>
					<input type="submit" id="newListSearchBtn" value="검색">
				</form>
			</div>
			<div id="newListMain">
...
$(document).ready(function(){
		...
		onPageClick : function(event,page) {
			...
			$(".page-link").on("click",function(event){ 
				...
				if(pageNo != "First" && pageNo != "Previous" && pageNo != "Next" && pageNo != "Last") {
					newListUrl = "newListUrl?pageNo=" + pageNo;
				}
				else if(pageNo == "Next") {
					pageA = $("li.active > a");
					pageNo = pageA.text();
					...
					pageNo2 = pageNo1 + 1;
					newListUrl = "newListUrl?pageNo=" + pageNo2; 
				}
					...
					newListUrl = "newListUrl?pageNo=" + 1; 
				}
				else if(pageNo == "Last") {
					newListUrl = "newListUrl?pageNo=" + 5; 
				}
				else {
					return;
				}
	...
	$("#newListSearch").submit(function(event){
		event.preventDefault();
		$.ajax({
			url : "newListSearch",
			type : "GET",
			data : $("#newListSearch").serialize(),
